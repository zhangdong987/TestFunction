...
public static class CreateSpriteAsset
{
    ...
    [MenuItem("Assets/Create/静态表情Asset(注意ID不一致)", false, 10)]
    static void main()
    {
    ...
    //创建动态表情
    [MenuItem("Assets/Create/动态表情Asset(注意ID不一致)", false, 10)]
    static void dynamicmain()
    {
        Object target = Selection.activeObject;
        if (target == null || target.GetType() != typeof(Texture2D))
            return;
        ...
        if (isNewAsset)
        {
            spriteAsset = ScriptableObject.CreateInstance<SpriteAsset>();
            spriteAsset.texSource = sourceTex;
            spriteAsset.listSpriteGroup = GetAssetSpriteInfor(sourceTex,false);
            AssetDatabase.CreateAsset(spriteAsset, filePath + fileNameWithoutExtension + ".asset");
        }
    }
